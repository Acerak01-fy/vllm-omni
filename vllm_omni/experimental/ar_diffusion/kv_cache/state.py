...
class ARDiffusionKVState:
    ...
    def get_cross_kv_caches(self, is_negative: bool) -> list[dict]:
        ...
        if not (self._cross_populated[is_negative] and self.kv_cache.cross_attn_length > 0):
            raise RuntimeError(
                f"AR-Diffusion cross-attn read before _kv_populate_cross "
                f"(neg={is_negative}, cross_attn_length={self.kv_cache.cross_attn_length}) "
                "- the engine must own all cross KV"
            )
    ...
    def reset(self) -> None:
        """Drop this session's KV and restart both branch adapters."""
